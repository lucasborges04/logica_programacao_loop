...
while(verif):
    total = 0
    cont = 1
    produtos = float(input(f'Produto {cont}: '))
    cont += 1
    while(produtos != 0):
        total += produtos
        produtos = float(input(f'Produto {cont}: '))
    ...
    while(dinheiro < total):
        falta = abs(troco)
        print(f'Está faltando R${falta:.2f}')
        dinheiro_faltando = float(input('Dinheiro: R$'))
        dinheiro += dinheiro_faltando
        troco += dinheiro_faltando
